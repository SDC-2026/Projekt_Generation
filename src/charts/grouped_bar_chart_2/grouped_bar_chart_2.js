...
const LIKERT_COLORS_2 = ['#2E6F40', '#81B69D', '#D7D3CC', '#D8B69C', '#B96A52'];
...
function initLikertChart2() {
  ...
  function render() {
    const currentGroup = LIKERT_DATA_2[likert_active_2];
    ...
    const rows = currentQuestions.map(row => {
      const bars = row.values.map((val, idx) => {
        // Berechnet das n für das einzelne Segment (z.B. n=36)
        const segmentN = Math.round((val * currentN) / 100);
        const tipText = `<strong>${LIKERT_TITLES_2[idx]}</strong><br>${val.toFixed(1)} % · n = ${segmentN}`;
